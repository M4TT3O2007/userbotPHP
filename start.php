...
if(file_exists("LatestV.txt")){
	unlink("LatestV.txt");
	}
if(file_exists("LatestV.json")){
	unlink("LatestV.json");
	}
...
echo 'PHPUserbot is ready.
Name: '.$me["first_name"].'
PHP Version: '.phpversion().'
phpUserbot version: 1.0.0';
    ...
    try {
      ...
      if ($settings['old_update_parser']) {
        ...
        if (isset($msg) and isset($chatID) and isset($type) and $msg) {
          ...
          } elseif ($type == 'channel') {} else {}
        }
      }
if(!empty($update['message']['out'])){
if($message['message']['out'] =! false){
